...
## load dataset
```{r}
data01 <- read_csv("Data/LHS_exercise_02102022.csv")
...
data02<- data01 %>% 
	mutate(anxiety = 
	case_when(phq1_1 >2 & phq2_1 >2 ~ "1", phq1_1 <3 ~ "0" ))
```


```{r}
data03<- data02 %>%  	mutate(depression =  	case_when(phq3_1 >2 & phq4_1 >2 ~ "1", phq1_1 <3 ~ "0" ))
...
```{r}
data05<- data01%>%
  mutate(distress = rowSums(across(phq1_1:phq4_1),
                              na.rm = TRUE))
```
...
```{r}
data05 <- data05%>%
  mutate(anxiety = case_when(phq1_1 + phq2_1 >= 2 ~ 1,
                           TRUE ~ 0),
